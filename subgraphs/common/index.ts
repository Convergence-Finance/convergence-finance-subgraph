...
import { usdt_conv_lp_address, eth_conv_lp_address } from "./envVars";
...
//排序必须和lpAddressArr 对应
export let lpNameArr: string[] = ["usdt_conv", "eth_conv"];

//排序必须和lpNameArr 对应
export let lpAddressArr: Address[] = [
  usdt_conv_lp_address,
  eth_conv_lp_address,
];

export function getkeyName(address: Address): string {
  let index = lpAddressArr.indexOf(address);
  let keyName = "usdt_conv";
  if (index >= 0) {
    keyName = lpNameArr[index];
  }
  return keyName;
}
